...
<template>
  ...
  >
    <h3 class="text-8 sm:text-12 md:text-15 lg:text-20 font-bold z-10">
      定制个人专属页面
    </h3>
    <div
    ...
    >
      ...
      >
        ...
        <input
          v-model="name"
          class="flex-1 h-full rounded-lg px-2 outline-none bg-transparent"
          type="text"
  ...
  >
    ...
    <div
      class="w-full md:w-1/2 flex flex-col items-center md:items-start mt-10 md:mt-0"
    >
      <span class="text-4 sm:text-5 text-white/60">
        现在就开始使用我的APP吧！
      </span>
      <div
        class="flex mt-5 md:mt-8 gap-6 justify-center items-center sm:gap-7 md:gap-8"
      >
        <router-link
          class="primary-button primary-button-text primary-button-color"
          to="/auth/register"
        >
          立即注册
        </router-link>
        <router-link class="~ h-full primary-button-text-flip" to="/auth/login">
          立即登录
        </router-link>
      </div>
    </div>
  ...
  <section
    class="container mx-auto px-0 my-15 sm:my-25 md:my-40 px-3 flex md:px-0 flex-col md:flex-row"
  >
    <div
      class="flex flex-col gap-3 sm:gap-6 md:gap-10 items-center mr-auto w-full md:w-auto"
    >
    ...
    <div class="w-full md:w-1/2 mt-10 md:mt-0">
      ...
      >
        <li v-for="i of 5" :key="i" class="bg-zinc-800 h-50 rounded-lg p-2">
          广告位招租
        </li>
      </ul>
    </div>
  ...
  >
    <div class="flex items-center gap-3">
      ...
      <span class="text-zinc-300 text-5 font-bold sm:text-6 md:text-7">
        个人专属页面
      </span>
    </div>
    <nav
      class="flex flex-col items-start sm:flex-row text-3 gap-5 sm:gap-10 md:gap-20 w-full sm:w-1/2"
    >
      <dl
      ...
      >
        <dt
          class="col-span-3 text-4 sm:text-5 md:text-6 mb-2 sm:mb-3 md:mb-5 text-zinc-300"
        >
          {{ item.title }}
        </dt>
        <dd
          class="flex sm:col-span-3 text-3 sm:text-4 md:text-5"
          v-for="i of item.nav"
          :key="i.title"
        >
