...
export const encrypt = (password: string): HashedPassword => {
	...
	const key = pbkdf2Sync(password, salt, iterations, keySize, digest).toString('hex');

	return [key, salt].join('.') as HashedPassword
}

export const decrypt = (password: string, hashedPassword: HashedPassword): boolean => {
	const [key, salt] = hashedPassword.split('.')

	const hashedInputPassword = pbkdf2Sync(password, salt, iterations, keySize, digest).toString('hex');
