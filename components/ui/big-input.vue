...
const validatorList =
  inject<ShallowRef<(() => Promise<unknown>)[]>>("big-form-key");
...
<template>
  <div
    class="h-10 sm:h-12 md:h-15 bg-zinc-800 px-1 rounded-lg relative focus-within:bg-purple-700/30 focus-within:ring-2 focus-within:ring-purple-900"
    :class="[
      wrapperClass,
      {
  ...
  >
    ...
    />
    <div
      class="text-rose-600 absolute bottom-0 left-0 text-3 sm:text-4 message"
      v-if="errorMessage != ''"
    >
      {{ errorMessage }}
    </div>
